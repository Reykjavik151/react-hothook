...
var useCached = function useCached(value) {
  ...
  react.useEffect(function () {
    if (!(lodash.isEmpty(value) || lodash.isUndefined(value) || lodash.isNull(value))) {
      ref.current = value;
    }
  }, [value]);
  return ref.current;
...
var useCountdown = function useCountdown(_ref) {
  ...
  var _useState = react.useState(initialRemainingTimeMs),
  ...
  var pauseCountdown = react.useCallback(function () {
    setIsPaused(true);
    stopTimer();
  }, [stopTimer]);
  var resumeCountdown = react.useCallback(function () {
    setIsPaused(false);
    startTimer();
  }, [startTimer]);
  var setNewRemainingTime = react.useCallback(function (newTimeMs) {
    setTimeLeft(newTimeMs);
  }, []);
  var resetCountdown = react.useCallback(function () {
    setTimeLeft(0);
    pauseCountdown();
  }, [pauseCountdown]);
  return {
...
var useDeepCompareEffect = function useDeepCompareEffect(callback, dependencies) {
  var currentDependenciesRef = react.useRef(null);
...
var useSpecificKeyExtractor = function useSpecificKeyExtractor(prefix, field) {
  return react.useCallback(function (item, index) {
...
var useStateWithValidation = function useStateWithValidation(initialValue, validator) {
  var _useState = react.useState(initialValue),
    state = _useState[0],
    setState = _useState[1];
  var _useState2 = react.useState(validator(state)),
...
var useToggle = function useToggle(defaultValue) {
  ...
  var toggleValue = react.useCallback(function () {
...
exports.useSpecificKeyExtractor = useSpecificKeyExtractor;
exports.useStateWithCached = useStateWithCached;
exports.useStateWithPrevious = useStateWithPrevious;
